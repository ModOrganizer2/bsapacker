...
namespace BsaPacker
{
	...
	{
		QStringList successfulArchiveNames;
		...
		for (auto&& type : types) {
			...
			for (auto& archive : archives) {
				if (archive) {
					const QString& archiveFullPath = this->m_ArchiveNameService->GetArchiveFullPath(type, modDto.get(), archiveIndex);
					...
					if (res) {
						...
						successfulArchiveNames.append(archiveFullPath.mid(archiveFullPath.lastIndexOf("\\") + 1));
						++archiveIndex;
					}
				}
			}
		...
		if (!successfulArchiveNames.isEmpty()) {
			QMessageBox::information(nullptr, "", QObject::tr("Created archive(s):\n\n") + successfulArchiveNames.join(",\n"));
		}
